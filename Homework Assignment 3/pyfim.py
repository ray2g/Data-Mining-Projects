...
#           2020.03.31 a pyeclat function added for comptibility with mlextend (afalcao)
#-----------------------------------------------------------------------
...
import pandas as pd
def pyeclat(trs, supp):
    # this is an extension by Andre Falcao to make the eclat output compatible
    # with mlextend. this function requires a list of transactions and a support value (<1.0)
    # returns a pandas dataframe with absolute support and the relevant itemsets 
    
    supp=supp*100
    transactions=[frozenset(tr) for tr in trs]
    res = eclat(transactions, supp=supp, out=[])
    itemsets=[]
    supports=[]
    N=len(transactions)
    for i_s, n in res:
        supports.append(n/N)
        itemsets.append(i_s)
    D = {"support": supports, "itemsets": itemsets}
...
if __name__ == '__main__':
    ...
    with open(fixed[0], 'rt', encoding = "ISO-8859-1") as inp:
